...
@Component({
  selector: 'app-recharge-bill-water',
  templateUrl: './recharge-bill-water.component.html',
  styleUrls: ['./recharge-bill-water.component.css'],
})
export class RechargeBillWaterComponent implements OnInit {
  result: any = '';
  result2: any = [];
  clicked: any = '';
  inputarr: any = [];
  ...
  rechargeBillWater: FormGroup = new FormGroup({});
  ...
  constructor(
    ...
    this.rechargeBillWater = this.fb.group({
  ...
  get getForm() {
    return this.rechargeBillWater.controls;
  }

  billWater(formData: any) {
    this.ValidationMsg = true;
    this.submitted = true;
    this.RechargeFormData = formData;
    if (this.rechargeBillWater.valid) {
      const data = { ...formData };
  ...
  operatorsId(value: any) {
    ...
    this.previousControls.forEach((e: string) => {
      this.rechargeBillWater.removeControl(e);
    });
    ...
    this.rechargeBillWater.removeControl(this.removeValue);
  }
  ...
  test1(element: any) {
    ...
    this.inputarr.forEach((input: any) => {
      ...
      this.rechargeBillWater.addControl(
        input.Name,
    ...
    // });
    // this.rechargeBillWater.removeControl('dynamic');
  }
}
