...
@Component({
  ...
  styleUrls: ['./lpg-gas.component.css'],
})
export class LpgGasComponent implements OnInit {
  result: any = '';
  result2: any = [];
  clicked: any = '';
  inputarr: any = [];
  ...
  login: boolean = false;
  lpgGasForm: FormGroup = new FormGroup({});
  ...
  get getForm() {
    return this.lpgGasForm.controls;
  }
  ...
  operatorsId(value: any) {
    ...
    this.previousControls.forEach((e: string) => {
      this.lpgGasForm.removeControl(e);
    });
    ...
    this.lpgGasForm.removeControl(this.removeValue);
  }
