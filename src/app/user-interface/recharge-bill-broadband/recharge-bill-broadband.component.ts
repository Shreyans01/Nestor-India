...
@Component({
  selector: 'app-recharge-bill-broadband',
  templateUrl: './recharge-bill-broadband.component.html',
  styleUrls: ['./recharge-bill-broadband.component.css'],
})
export class RechargeBillBroadbandComponent implements OnInit {
  result: any = '';
  result2: any = [];
  clicked: any = '';
  inputarr: any = [];
  ...
  login: boolean = false;
  rechargeBillBroadband: FormGroup = new FormGroup({});
  ...
  constructor(
    ...
    this.rechargeBillBroadband = this.fb.group({
  ...
  get getForm() {
    return this.rechargeBillBroadband.controls;
  }

  billBroadband(formData: any) {
    this.submitted = true;
    this.ValidationMsg = true;
    this.RechargeFormData = formData;
    if (this.rechargeBillBroadband.valid) {
      const data = { ...formData };
  ...
  operatorsId(value: any) {
    ...
    this.previousControls.forEach((e: string) => {
      this.rechargeBillBroadband.removeControl(e);
    });
    ...
    this.rechargeBillBroadband.removeControl(this.removeValue);
  }
  ...
  test1(element: any) {
    ...
    this.inputarr.forEach((input: any) => {
      ...
      this.rechargeBillBroadband.addControl(
        input.Name,
    ...
    // });
    // this.rechargeBillBroadband.removeControl('dynamic');
  }
}
