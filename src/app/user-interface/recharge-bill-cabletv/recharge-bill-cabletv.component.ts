...
@Component({
  selector: 'app-recharge-bill-cabletv',
  templateUrl: './recharge-bill-cabletv.component.html',
  styleUrls: ['./recharge-bill-cabletv.component.css'],
})
export class RechargeBillCabletvComponent implements OnInit {
  result: any = '';
  result2: any = [];
  clicked: any = '';
  inputarr: any = [];
  ...
  login: boolean = false;
  rechargeBillCableTv: FormGroup = new FormGroup({});
  ...
  constructor(
    ...
    this.rechargeBillCableTv = this.fb.group({
  ...
  get getForm() {
    return this.rechargeBillCableTv.controls;
  }

  billCableTv(formData: any) {
    this.ValidationMsg = true;
    this.submitted = true;
    this.RechargeFormData = formData;
    if (this.rechargeBillCableTv.valid) {
      const data = { ...formData };
  ...
  operatorsId(value: any) {
    ...
    this.previousControls.forEach((e: string) => {
      this.rechargeBillCableTv.removeControl(e);
    });
    ...
    this.rechargeBillCableTv.removeControl(this.removeValue);
  }
  ...
  test1(element: any) {
    ...
    this.inputarr.forEach((input: any) => {
      ...
      this.rechargeBillCableTv.addControl(
        input.Name,
    ...
    // });
    // this.rechargeBillCableTv.removeControl('dynamic');
  }
}
