...
export class FastagComponent implements OnInit {
  ...
  FastagForm: FormGroup = new FormGroup({});
  ...
  get getForm() {
    return this.FastagForm.controls;
  }

  Fastag(formData: any) {
    this.submitted = true;
    this.RechargeFormData = formData;
    if (this.FastagForm.valid) {
      const data = { ...formData };
      ...
      if (this.show == false) {
        ...
        this.userData1.setpromocodeType('Fastag');
  ...
  operatorsId(value: any) {
    ...
    this.previousControls.forEach((e: string) => {
      this.FastagForm.removeControl(e);
    });
    ...
    this.FastagForm.removeControl(this.removeValue);
  }
  ...
  test1(element: any) {
    ...
    this.inputarr.forEach((input: any) => {
        ...
        this.AmountValidation = input.Name;
