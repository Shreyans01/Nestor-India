...
@Component({
  selector: 'app-recharge-bill-gas',
  templateUrl: './recharge-bill-gas.component.html',
  styleUrls: ['./recharge-bill-gas.component.css'],
})
export class RechargeBillGasComponent implements OnInit {
  result: any = '';
  result2: any = [];
  clicked: any = '';
  inputarr: any = [];
  ...
  amounts: boolean = true;
  ValidationMsg: boolean = false;
  login = false;
  isBillAmount: boolean = false;
  rechargeBillGas: FormGroup = new FormGroup({});
  ...
  constructor(
    ...
    this.rechargeBillGas = this.fb.group({
  ...
  get getForm() {
    return this.rechargeBillGas.controls;
  }

  billGas(formData: any) {
    this.ValidationMsg = true;
    this.submitted = true;
    this.RechargeFormData = formData;
    if (this.rechargeBillGas.valid) {
      const data = { ...formData };
  ...
  operatorsId(value: any) {
    ...
    this.previousControls.forEach((e: string) => {
      this.rechargeBillGas.removeControl(e);
    });
    ...
    this.rechargeBillGas.removeControl(this.removeValue);
  }
  ...
  test1(element: any) {
    ...
    this.inputarr.forEach((input: any) => {
      ...
      this.rechargeBillGas.addControl(
        input.Name,
    ...
    // });
    // this.rechargeBillGas.removeControl('dynamic');
  }
}
